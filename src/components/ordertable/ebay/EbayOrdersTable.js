...
export default function EbayOrdersTable() {
  ...
  const { response, error, loading, setLoading } = useFetch(
    `ebay/mete?orderStatus=${buttonTag}&limit=${rowsPerPage}&offset=${
      page * rowsPerPage
  ...
  return (
    <TableContainer component={Paper} className={classes.tContainer}>
      <div className={classes.topDiv}>
        <h2 className={classes.headerStyle}>Ebay Orders</h2>
      ...
      <TopFullfilmentTypeButtonGroup
        ...
        orderStatusTags={ebayFullfilmentTypes}
      />
      <TopButtonGroup
        buttonTag={buttonTag}
        handleTagBtnClick={handleTagBtnClick}
        orderStatusTags={EBAY_OrderStatus}
      />
